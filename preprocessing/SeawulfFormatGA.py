...
def read_geojson():
    precincts_gdf = gpd.read_file("GAPrecinctFinal.geojson")
    precincts_gdf = precincts_gdf.drop(columns=["geometry"])
    return precincts_gdf

def createNodeObject(precinct):
    node = {}
    node = precinct.to_dict()
    node["id"] = precinct["UNIQUE_ID"]
    return node
...
gdf = read_geojson()
add_to_graph(gdf)
with open('GA_graph.json', 'w') as file:
