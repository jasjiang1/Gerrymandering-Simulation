...
function SecondStateAssembly({mapSelection, setlayerhighlight}) {
    ...
    useEffect(() => {
      ...
      useEffect(()=>{
        let table = document.getElementById("assemblytabletwo");
        table.innerHTML = "";
        ...
        document.getElementById("assemblytabletwo").appendChild(columns);
        if(assemblyReps != false ){
           for(let index =0; index<assemblyReps.length;index++){
            ...
            document.getElementById("assemblytabletwo").appendChild(row);
           }
        }
      ...
      return (
       ...
       <table id = 'assemblytabletwo'>
        </table>
      </div>
    );
}

export default SecondStateAssembly;
