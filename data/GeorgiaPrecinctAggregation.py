...
def updateGeoJson():
    ...
    with open('CensusBlockGeorgia.geojson', 'w') as f:
        json.dump(geojson_data, f)
    print("GEOJSON For Census Block Demographics Completed")

def aggregateCensusBlockToPrecincts():
    census_blocks_gdf = gpd.read_file('CensusBlockGeorgia.geojson')
    precincts_gdf = gpd.read_file('ga_2022_gen_st_prec.geojson')
    ...
    aggregated_data = joined_gdf.groupby('UNIQUE_ID').agg({
        ...
        'Democratic': 'sum',
        'Republican': 'sum',
        'StateAssemblyElection2022': 'sum',
        'StateAssemblyElectionWhiteVoters2022': 'sum',
        'StateAssemblyElectionHispanicVoters2022': 'sum',
        'StateAssemblyElectionAfricanAmericanVoters2022': 'sum',
        'StateAssemblyElectionAsianVoters2022': 'sum',
    ...
    final_gdf = precincts_gdf.merge(aggregated_data, left_on='UNIQUE_ID', right_on='UNIQUE_ID')
    final_gdf.to_file('GAPrecinctsData.geojson', driver='GeoJSON')
